...
/**
 * When a new version of Niord Proxy is rolled out, aggressive "cachers", such as Chrome,
 * may not update partial HTML files (loaded e.g. from directives).
 * <p>
        ...
        if (isChrome(request)) {
            ...
            if (file != null) {
                ...
                if (evaluatePreconditions(etag, request, response)) {
                    log.log(Level.FINER, "ETag match for path " + servletPath);
                    return;
                ...
                log.log(Level.FINER, "No ETag match for path " + servletPath);
